...
config.getUpdateData = function(mode, data) {
  if (mode === 'module') {
      console.log(`import ${data.pageNameUpper} from '../pages/${data.pageNameUpper}'`);
      return [{
        path: path.resolve(process.cwd(), 'app/containers/App/index.js'),
        position: {
    ...
    // }, {
    //     path: path.resolve(process.cwd(), 'app/utils/menu.config.js'),
    //     position: {
    //         name: 'entry',
    //         type: 'ObjectExpression',
    //         only: true,
