...
export class OrderDataPage extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  ...
  render() {
    ...
    return (
      <article>
        <Helmet>
          <title>{intl.formatMessage(messages.databackend2.header.title)}</title>
          <meta name="description" content={intl.formatMessage(messages.databackend2.header.content)} />
        </Helmet>
        <div>
          <Toolbar />
          <DataTable />
